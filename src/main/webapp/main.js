const API_BASE = "http://localhost:8080/lab1-1.0-SNAPSHOT/api/spacemarines";
const API_CHAPTERS = "http://localhost:8080/lab1-1.0-SNAPSHOT/api/chapters";
const API_COORDS = "http://localhost:8080/lab1-1.0-SNAPSHOT/api/coordinates";
    ...
    try {
        ...
        sortedMarines.forEach(m => {
            ...
            row.innerHTML = `
                ...
                <td class="editable dropdown-field" data-field="coordinatesId" data-original="${m.coordinates?.id ?? ''}">${m.coordinates ? `x:${m.coordinates.x}, y:${m.coordinates.y}` : "-"}</td>
                <td class="editable dropdown-field" data-field="chapterId" data-original="${m.chapter?.id ?? ''}">${m.chapter?.name ?? "-"}</td>
            `;
    ...
    if (field === 'coordinatesId' || field === 'chapterId') {
        try {
            ...
            if (field === 'coordinatesId') {
                apiUrl = 'http://localhost:8080/lab1-1.0-SNAPSHOT/api/coordinates';
            } else {
                apiUrl = 'http://localhost:8080/lab1-1.0-SNAPSHOT/api/chapters';
            }
...
async function updateMarineInline(marineId, updatedData, cell, displayValue, actualValue) {
    try {
        ...
        const updatePayload = {
            ...
            coordinatesId: updatedData.coordinatesId !== undefined ? (updatedData.coordinatesId ? parseInt(updatedData.coordinatesId) : null) : fullMarineData.coordinates?.id,
            chapterId: updatedData.chapterId !== undefined ? (updatedData.chapterId ? parseInt(updatedData.chapterId) : null) : fullMarineData.chapter?.id
        };
